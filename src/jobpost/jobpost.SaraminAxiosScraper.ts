...
export class SaraminScraper {
    ...
    async getDataAsHtml() {
        ...
        jobData.each(async function (i: number, elem: any) {
            ...
            const addressUpper = data(elem)
                .find('.job_condition > span:nth-child(1) > a:nth-child(1)')
                .text()
            const addressLower = data(elem)
                .find('.job_condition > span:nth-child(1) > a:nth-child(2)')
                .text()
            const career = data(elem)
                .find('.job_condition > span:nth-child(2)')
                .text()
            const education = data(elem)
                .find('.job_condition > span:nth-child(3)')
                .text()
            const worktype = data(elem)
                .find('.job_condition > span:nth-child(4)')
                .text()
