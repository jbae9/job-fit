...
export class SaraminSelenium {
	...
	async getSaraminScraper() {
		...
		const driver = await new Builder()
			...
			.build()
		const saraminScraper = new SaraminScraper(`https://www.saramin.co.kr/zf_user/jobs/list/job-category?page=1&cat_mcls=2&isAjaxRequest=0&page_count=` + this.pageCount + `&sort=RL&type=job-category&is_param=1&isSearchResultEmpty=1&isSectionHome=0&searchParamCount=1#searchTitle`);
			...
			for (let i = 0; i < allJobsArr.length; i++) {
				...
				if (salaryText === '급여') {
					salary = await allJobs
						...
						.getText()
					if (salary === '면접 후 결정' || salary === '회사내규에 따름') {
						salary = '0'
					} else {
				...
				allJobsArr[i].deadlineDtm.indexOf('채용') == -1 ? null : new Date(String(new Date().getFullYear) + '/' + String(allJobsArr[i].deadlineDtm).substring(2, 7))
				...
				try {
					...
					for (let j = 0; j < companyOptionList.length; j++) {
						const option = await companyOptionList[j].findElement(By.css('dt')).getText()
						const optionText = await companyOptionList[j].findElement(By.css('dd')).getText()
						for (const key in companyOption) {
							if (option.indexOf(key) !== -1) {
								allCompanies[i][`${companyOption[key]}`] = optionText
								break
							}
						}
					}
				...
				if (allCompanies[i]["numberEmployees"] != null) {
					allCompanies[i]["numberEmployees"] = Number(allCompanies[i]["numberEmployees"].split(' ', 1)[0])
				}
				if (allCompanies[i]["foundedYear"] != null) {
					allCompanies[i]["foundedYear"] = allCompanies[i]["foundedYear"]?.split('(')[0]
				}
